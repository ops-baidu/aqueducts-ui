...
aqueductsApp.controller('OrgServiceController', ['$modal', '$route','$log','$scope', '$routeParams', '$location','Restangular', '$window', function($modal, $route, $log, $scope, $routeParams, $location, Restangular, $window) {
  $scope.create = function () {
    ...
    services.post(a_service).then(function() {
      $location.path('/orgs/'+$scope.orgname);
  ...
  $scope.initService = function(){
    ...
    $scope.service = { demo: false, label: 'New organization service', name: '', huabei: true, huadong: false, selectFlow_huadong: 1, selectFlow_huabei: 1};
  };
