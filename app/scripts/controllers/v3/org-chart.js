...
aqueductsApp.controller('OrgChartController', [ '$scope','$http', '$q', '$routeParams', 'Restangular', 'EventsApiBaseUrl' , function ($scope,$http,$q,$routeParams,Restangular,EventsApiBaseUrl) {
  ...
  $scope.initConfig=function(config,orgname,service_name,job) {
    ...
    $q.all(promises).then(function(results) {
      ...
      var calc = results[1] ;
      var series = [  
        '/v1/' + 'events?product=' + orgname + '&service=' + service_name + '&item=' + item.name + '&calculation=' + calc.name + '&from=-24h&to=now&period=60&name=today&diff=0', 
        '/v1/' + 'events?product=' + orgname + '&service=' + service_name + '&item=' + item.name + '&calculation=' + calc.name + '&from=-48h&to=-24h&period=60&name=yesterday&diff=86400', 
        '/v1/' + 'events?product=' + orgname + '&service=' + service_name + '&item=' + item.name + '&calculation=' + calc.name + '&from=-168h&to=-144h&period=60&name=lastweek&diff=518400',
      ] ; 
  ...
  $q.all(promises).then(function(results) {
    ...
    var promise = [ $http.get('/v1/events?product=' + org.name + '&service=' + service.name + '&item=page_view&calculation=count&from=-16s&to=now')] ;
            ...
            events: {
                load: function() {
                    ...
                    setInterval(function() {
                        var request = '/v1/events?product=' + org.name + '&service=' + service.name + '&item=page_view&calculation=count&from=-16s&to=now' ;
                        $http.get(request).success(function (data, status, headers, config) {
