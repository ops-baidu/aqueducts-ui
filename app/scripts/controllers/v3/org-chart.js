...
aqueductsApp.controller('OrgChartController', [ '$scope','$http', '$q', '$routeParams', 'Restangular', 'EventsApiBaseUrl' , function ($scope,$http,$q,$routeParams,Restangular,EventsApiBaseUrl) {
  ...
  $scope.showCharts = function(service_name){
    ...
    var promises = [
        Restangular.one('orgs', orgname).get(),
        Restangular.one('orgs', orgname).one('services', service_name).get()
    ];
    $q.all(promises).then(function(results) {
      var org = results[0] ;
      var service = results[1] ;
      var promise = [ $http.get(EventsApiBaseUrl + 'events?product=' + org.name + '&service=' + service.name + '&item=page_view&calculation=count&from=-16s&to=now')] ;
            ...
            events: {
              load: function() {
                ...
                setInterval(function() {
                  var request = EventsApiBaseUrl + 'events?product=' + org.name + '&service=' + service.name + '&item=page_view&calculation=count&from=-16s&to=now' ;
                  $http.get(request).success(function (data, status, headers, config) {
