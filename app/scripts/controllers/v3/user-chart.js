...
aqueductsApp.controller('UserChartController', [ '$scope','$http', '$q', '$routeParams', 'Restangular', 'EventsApiBaseUrl' , function ($scope,$http,$q,$routeParams,Restangular,EventsApiBaseUrl) {
  ...
  $scope.showCharts = function(service_name){
    ...
    var promises = [
        Restangular.all('user').customGET('info').get(),
        Restangular.all('user').one('services', service_name).get()
    ];
    $q.all(promises).then(function(results) {
      var user = results[0] ;
